"""deal_engine — FastAPI entry point.

RENAMED 2026-09-09, from `gex_pf_engine`. It shared that name with the SIBLING
repo `files/gex_pf_engine`, which is the Gabillon project-finance engine serving
:8001 (cfads.py, waterfall.py, debt/sculpting.py). The two share not one module
name — this one computes deals (pre-COD, phases, ratios, COD test, TEA adapter);
that one computes cashflows. Anyone deleting either as "the duplicate" would
take out a service.

This service has no launch.json entry and no caller. It is dormant, not dead.
...
from deal_engine.auth.gex_jwt import AuthenticatedUser, get_current_user
from deal_engine.routes import deals_router
