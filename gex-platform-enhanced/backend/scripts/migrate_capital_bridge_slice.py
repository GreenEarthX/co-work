...
SLICE_TABLES: dict[str, tuple[str, ...]] = {
    "fuel_defaults": ("fuel_type",),
    "project_control": ("project_id",),
    "capital_stack_tranches": ("tranche_id",),
    "spend_wave": ("row_id",),
    "drawdown_quarters": ("row_id",),
    "personnel_plan": ("row_id",),
    "post_cod_schedule": ("row_id",),
    "dfi_criteria_status": ("row_id",),
    "development_packages": ("package_id",),
    "development_package_events": ("event_id",),
    "package_evidence": ("id",),
}
SEQUENCE_TABLES = ("package_evidence",)
...
def main() -> int:
    ...
    # ── Hash-chain verification (the point of this slice) ───────────────────
    if args.execute:
        import hashlib
        from collections import defaultdict

        def chain_state(fetch_all):
        ...
        src_rows = lite.execute(
            "SELECT * FROM development_package_events ORDER BY created_at, rowid").fetchall()
        with engine.connect() as pg:
            pg.execute(text("SET app.current_company_id = 'PLATFORM_ADMIN'"))
            dst_rows = pg.execute(text(
                "SELECT * FROM development_package_events "
                "ORDER BY created_at, event_id")).mappings().all()

        s_unver, s_links = chain_state(src_rows)
        d_unver, d_links = chain_state(dst_rows)
        print("\n══ HASH CHAIN ══")
        print(f"  sqlite  : {len(src_rows)} events, {s_unver} unverifiable, {s_links} broken links")
        print(f"  postgres: {len(dst_rows)} events, {d_unver} unverifiable, {d_links} broken links")
        if (s_unver, s_links, len(src_rows)) != (d_unver, d_links, len(dst_rows)):
            problems.append(
                f"chain state changed: sqlite=({s_unver},{s_links},{len(src_rows)}) "
                f"postgres=({d_unver},{d_links},{len(dst_rows)})")
        else:
            print("  chain state reproduced EXACTLY (including the 7 pre-existing "
                  "unverifiable rows — neither repaired nor worsened)")
