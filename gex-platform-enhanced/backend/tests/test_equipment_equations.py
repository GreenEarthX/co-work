...
def test_the_store_never_creates_equipment_equations_on_postgres(monkeypatch):
    """`equipment_equations` belongs to migration 050, policy included.
    ...
    def _explode(*a, **kw):
        raise AssertionError(
            "init_db() opened a PostgreSQL connection — equipment_equations belongs to "
            "migration 050, not to the runtime")

    monkeypatch.setattr(equations_store, "_equations_is_postgres", lambda: True)
    monkeypatch.setattr(equations_store, "workspace_connection", _explode)
    equations_store.init_db()
