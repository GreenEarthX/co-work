// Screen: Help text config (no screen)
/**
...
export const HELP = {
  ...
  LLCR: "Loan Life Coverage Ratio = NPV of remaining CFADS ÷ outstanding debt balance. Must exceed 1.40x at all times.",
  WACC: "Weighted Average Cost of Capital — blended cost of all capital tranches. Lower = cheaper financing = higher equity IRR.",
  WACC_SAVING:
  ...
  DSRA: "Debt Service Reserve Account — must hold 6 months of debt service as a liquidity buffer. Underfunding triggers a distribution lock-up.",
  ETV: "Enterprise Technology Value — optionality not on the balance sheet: product switching (H₂→NH₃→SAF→eMeOH), carbon premium escalation, GoO appreciation, learning curve, refinancing option at COD.",
  ...
  EVIDENCE_HIERARCHY_EFFECTIVE:
    "Gates are displayed with all G0-G11 rows visible and ranked by effective score. Effective score = raw gate score × mean evidence-verification weight, where AUDITED = 1.00, CONFIRMED = 0.85, SUBMITTED = 0.50, UNVERIFIED = 0.25. This highlights where weak evidence quality, not only low completion, is degrading the gate.",
  EVIDENCE_HIERARCHY_GATE_COVERAGE:
    "Gate coverage shows which visible projects contribute to a gate under current ABAC, RLS, and CISO rights, including molecule and geography scope. Use it to ask solution-oriented questions: which project is driving the low score, who owns the blocker, which evidence is still only submitted or unverified, and whether the issue is local to one contract or systemic across the portfolio.",
