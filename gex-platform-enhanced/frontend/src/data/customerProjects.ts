...
/**
 * Grid topology of the power supply — drives WHICH evidence G1 demands.
 *
 * It does NOT decide whether power evidence is owed: every model owes it. Off-
 * grid changes the instrument, not the obligation (see PowerAttribution).
 *  - OFF_GRID_BTM:    dedicated behind-the-meter generation, no grid import.
 *                     Evidence = direct-line route: additionality, temporal and
 *                     geographic correlation, zero-grid-import metering, and
 *                     retired attribute certificates. A PPA is still required
 *                     whenever the generator is a separate legal entity —
 *                     RED III defines a renewables PPA contractually, not
 *                     topologically.
...
/**
 * Evidence that the electricity behind the molecule actually qualifies it.
 *
 * Off-grid behind-the-meter is not an exemption — it is a different, and in
 * places stricter, evidence burden:
 *  - RFNBO DA (EU) 2023/1184 Art. 3 — the direct-line route still owes
 *    additionality, temporal and geographic correlation, AND adds a
 *    zero-grid-import metering condition the PPA route does not carry. Any
 *    grid import voids the route outright.
 *  - RFNBO DA Art. 5 — a generator that received operating or investment aid
 *    is disqualified for additionality unless the aid was fully repaid.
 *  - US 45V final regulations (Jan 2025) — EACs are required for captive,
 *    co-located behind-the-meter power, not only for purchased power, because
 *    BTM power can create induced emissions if it previously served the grid
 *    or another load. Incrementality, temporal matching and deliverability all
 *    apply to a dedicated on-site wind farm.
 *  - RED III Art. 2 — a "renewables power purchase agreement" is a contract to
 *    purchase renewable electricity DIRECTLY FROM an electricity producer. A
 *    direct line between two legal entities is therefore a PPA; only single-
 *    entity ownership of both assets escapes the contract, and it still owes
 *    certificate attribution.
 */
export interface PowerAttribution {
  /** Ownership of the generating asset relative to the fuel producer.
   *  SAME_ENTITY is the only case with no supply contract — and it still owes
   *  certificate attribution under 45V. */
  generator_ownership?: "SAME_ENTITY" | "AFFILIATE" | "THIRD_PARTY";
  /** Commissioning year of the generating asset. Additionality tests this
   *  against production start: the generator must have come into operation no
   *  more than 36 months before the fuel plant starts producing. */
  generator_cod_year?: number;
  /** Generator received operating or investment aid — disqualifying for RFNBO
   *  additionality unless fully repaid (DA Art. 5). */
  generator_received_aid?: boolean;
  /** Smart metering evidencing zero grid import on the direct-line route
   *  (DA Art. 3(b)). VERIFIED = metered and attested, not merely fitted. */
  grid_import_metering?: "NONE" | "INSTALLED" | "VERIFIED";
  /** Granularity at which attribute certificates (EACs / GOs) are retired
   *  against production. 45V requires hourly matching from 2028; RFNBO from
   *  2030. Annual matching is not sufficient for either beyond those dates. */
  certificate_matching?: "NONE" | "ANNUAL" | "MONTHLY" | "HOURLY";
}
...
export interface EnergyInput {
  ...
  /** Required (≥1) when power_model is GRID_CONNECTED or HYBRID, and whenever
   *  the generator is a separate legal entity on any topology. */
  ppas?: PPA[];
  /** Evidence qualifying the electricity. Owed on every power_model — off-grid
   *  swaps the instrument, not the obligation. Absent = undeclared, itself a gap. */
  power_attribution?: PowerAttribution;
...
export const CUSTOMER_PROJECTS: CustomerProject[] = [
  ...
  // Verification report 2026-05-21: corrected Status, capacity, COD, added RFOcean offtake
  {
    ...
    offtakes: [
      {
        ...
        notes:
          "Binding 10-year fixed-price. Structured to support project finance. Volume undisclosed but material. 8 methanol-ready tankers on order.",
      },
      {
        party: "ETFuels SA",
        binding_status: "MOU",
        ...
        notes:
          "Related-party (owner = offtaker). 5-year ToP, CIF Rotterdam. Not in public sources — may be aspirational or for a different facility.",
      },
      {
        ...
        verification: "UNVERIFIED",
        notes:
          "Indicative / non-binding expression of interest. Not in public sources.",
      },
    ],
    // Narrative-feeding structured fields (Option A — Hidalgo causal compression)
    // Off-grid behind-the-meter per public sources (grid-independent design).
    // BTM does not remove the power-evidence burden: 45V is claimed here, and
    // the final regulations require EACs for captive co-located power. The
    // attribution record below is the direct-line evidence, not a PPA substitute.
    energy_input: {
      power_mw: 340,
      source: "wind",
      power_model: "OFF_GRID_BTM",
      ppas: [],
      // Only what is defensible from public sources. Generator ownership, its
      // commissioning year and aid status are NOT public for this project, so
      // they stay undeclared rather than invented — undeclared is itself the
      // finding the engine should report. Metering and certificates are NONE
      // because the plant is pre-COD: neither can exist before it runs.
      power_attribution: {
        grid_import_metering: "NONE",
        certificate_matching: "NONE",
      },
    },
    ...
    certifications: [
      { scheme: "45V", status: "ACTIVE", tier: "Tier 1" },
      {
        scheme: "RFNBO",
        status: "UNDER_REVIEW",
        note: "temporal correlation review",
      },
    ],
    incentives: [
      {
    ...
    bankability: {
      ...
      next_milestone:
        "Resolve capacity discrepancy (GEX 165k vs public 120k t/yr) → then complete G4 credit support",
      ...
      risk_flags: [
        {
          ...
          source_ids: [
            "SRC_ETFUELS_OUR_SOLUTION_2026",
            "SRC_ARGUS_RFOCEAN_2026_02_24",
            "SRC_CARBONSTORAGE_RATTLESNAKE",
          ],
          status: "ACKNOWLEDGED",
        },
        {
      ...
      gates: [
        {
          ...
          blocking_items: [],
          description:
            "Surface rights, mineral rights waiver, access road easements",
        },
        {
        ...
        },
        {
          ...
          description:
            "PEM electrolyser OEM selection, EPC contract, technology wrap",
        },
        {
        ...
        },
        {
          ...
          blocking_items: [
            ...
            "LENDER_TECHNICAL_ADVISOR_SIGN_OFF",
          ],
          description:
            "IEA-compliant CFADS model, DSCR stress test, audited by Deloitte PF",
        },
        {
          ...
          is_complete: false,
          blocking_items: [
            "SENIOR_DEBT_COMMITMENT_LETTER",
            "EQUITY_FUNDING_CONFIRMATION",
          ],
          description:
            "All CPs for financial close — requires G4, G7, G8 complete",
        },
        {
    ...
    bankability: {
      ...
      risk_flags: [
        ...
        },
        {
          ...
          classification: "PUBLIC",
          source_ids: [
            "SRC_ETFUELS_RATTLESNAKE_FEED_2025_10_27",
            "SRC_ARGUS_RFOCEAN_2026_02_24",
          ],
          status: "OPEN",
        },
        {
          ...
          classification: "PUBLIC",
          source_ids: [
            "SRC_ETFUELS_RFOCEAN_2026_02_24",
            "SRC_ARGUS_RFOCEAN_2026_02_24",
          ],
          status: "OPEN",
        },
      ...
      gates: [
        {
          ...
          description:
            "West Texas site — land control and water rights evidence required",
        },
        {
          id: "G1_GRID_UTILITIES_REALITY",
          name: "Power, Water & Critical Utility Access",
          ...
          description:
            "OFF_GRID_BTM — BTM generation asset spec, P90 yield study, grid-independence note and water permit pathway outstanding",
        },
        {
          id: "G3_INPUTS_SECURED",
          name: "Technology Vendor Locked",
          ...
          is_complete: false,
          blocking_items: [
            "ELECTROLYSER_SUPPLY_AGREEMENT",
            "METHANOL_TECH_LICENCE",
          ],
          description:
            "John Cockerill electrolyser + JM eMERALD selected; S&B FEED engineer; supply agreements pending",
        },
        {
          id: "G4_OFFTAKE_BANKABLE",
          name: "Bankable Offtake Agreement",
          ...
          description:
            "RFOcean binding fixed-price from 2030 — volume and redacted terms not yet filed",
        },
        {
          id: "G11_COD_STABILIZATION",
          name: "GHG Pathway Verified",
          ...
          is_complete: false,
          blocking_items: [
            "45V_RFNBO_PRE_ASSESSMENT",
            "FUELEU_COMPLIANCE_NOTE",
          ],
          description:
            "45V + RFNBO + FuelEU Maritime certification targets — pre-assessment required",
        },
      ],
      capital_status: [
        {
  ...
  // ── 0c · e-Methanol · Ranua, Finland (ETFuels — Näätäaapa) ─────────────
  {
    ...
    bankability: {
      ...
      next_milestone:
        "Secure land control and wind power evidence; begin RFNBO pre-assessment",
      unlocked_capital: ["GRANTS_TA"],
      risk_alerts: [
      ...
      risk_flags: [
        {
          ...
          classification: "PUBLIC",
          source_ids: [
            "SRC_ETFUELS_FINLAND_TAX_CREDIT_2026_02_23",
            "SRC_ETFUELS_OLD_PROJECTS",
          ],
          status: "OPEN",
        },
        {
      ...
      gates: [
        {
          ...
          description:
            "Ranua / Näätäaapa site — land control and Finnish permitting required",
        },
        {
          id: "G1_GRID_UTILITIES_REALITY",
          name: "Power, Water & Critical Utility Access",
          ...
          description:
            "OFF_GRID_BTM — 300 MW wind BTM: generation asset evidence, yield study, backup power and water source plan outstanding",
        },
        {
          id: "G3_INPUTS_SECURED",
          name: "Technology Vendor Locked",
          ...
          description:
            "Renewable-hydrogen-based e-methanol — technology details sparse in public domain",
        },
        {
          id: "G4_OFFTAKE_BANKABLE",
          name: "Bankable Offtake Agreement",
          ...
          description:
            "Target: European maritime + industrial — no offtake publicly disclosed",
        },
        {
          id: "G11_COD_STABILIZATION",
          name: "GHG Pathway Verified",
          ...
          description:
            "EU RFNBO + RED III + FuelEU Maritime + Finnish clean transition targets",
        },
      ],
      capital_status: [
        {
  ...
  // ── 0d · SAF · Redcar, UK (ETFuels — SkyFuel Teesside) ─────────────────
  {
    ...
    bankability: {
      ...
      next_milestone:
        "Secure Teesside land control and green H₂ partnership contract",
      unlocked_capital: ["GRANTS_TA"],
      risk_alerts: [
      ...
      gates: [
        ...
        },
        {
          ...
          is_complete: false,
          blocking_items: [
            "MtJ_TECHNOLOGY_LICENCE",
            "GREEN_H2_PARTNERSHIP_CONTRACT",
            "CO2_CONTRACT",
          ],
          description:
            "Johnson Matthey MtJ + Protium H₂ — partnership contracts pending",
        },
        {
          id: "G4_OFFTAKE_BANKABLE",
          name: "Bankable Offtake Agreement",
          ...
          description:
            "Target: UK + EU aviation — no offtake publicly disclosed",
        },
        {
          id: "G11_COD_STABILIZATION",
          ...
          is_complete: false,
          blocking_items: [
            "SAF_CERTIFICATION_PATHWAY",
            "ASTM_BLENDING",
            "RCM_ELIGIBILITY",
          ],
          description:
            "UK SAF Mandate + Revenue Certainty Mechanism + CORSIA + ASTM approval chain",
        },
      ],
      capital_status: [
        {
  ...
  // ── 1 · H2 · Bremen, Germany ─────────────────────────────────────────────
  {
    ...
    description:
      "PEM electrolysis plant fed by North Sea offshore wind PPAs. Primary offtake: industrial H2 for port decarbonisation and heavy transport.",
    ...
    energy_input: {
      ...
      ppas: [
        {
          counterparty: "North Sea OWF Consortium",
          volume_mw: 200,
          price_eur_mwh: null,
          tenor_years: 15,
          status: "SIGNED",
    ...
    bankability: {
      ...
      next_milestone:
        "Finalise grid connection agreement (G1) to unlock Strategic Equity",
      ...
      gates: [
        {
          ...
          is_complete: true,
          blocking_items: [],
          description: "Land title and environmental permits",
        },
        {
          id: "G1_GRID_UTILITIES_REALITY",
          name: "Power, Water & Critical Utility Access",
          ...
          description:
            "Grid-connected: PPA signed ✓ · interconnection study ✓ · queue position ✓ · curtailment assessed. Blocking: connection cost, COD compatibility, PPA coverage analysis",
        },
        {
          ...
          description: "Fixed-price EPC contract with performance guarantees",
        },
      ],
      capital_status: [
        {
        ...
        },
        {
          ...
          amount: "€55M",
          is_unlocked: false,
          gating_gates: ["G1_GRID_UTILITIES_REALITY"],
          progress_pct: 67,
          commitment_status: "INDICATIVE",
        },
        {
          ...
          amount: "€135M",
          is_unlocked: false,
          gating_gates: ["G5_EPC_RISK_PRICED", "G1_GRID_UTILITIES_REALITY"],
          progress_pct: 73,
          commitment_status: "NONE",
        },
      ],
  ...
  // ── 2 · NH3 · Rotterdam, Netherlands ─────────────────────────────────────
  {
    ...
    description:
      "Green ammonia production integrated with Maasvlakte port logistics. Offtake committed to fertiliser manufacturers and export traders.",
    owner_company: "RotterdamGreenFuels BV",
    associated_companies: ["Allianz", "Zürich Versicherung AG"],
    bankability: {
      overall_completion: 52,
      next_milestone:
        "Secure bankable off-take agreement (G4) to unlock €120M Strategic Equity",
      unlocked_capital: ["GRANTS_TA"],
      risk_alerts: [
      ...
      gates: [
        {
          ...
          description: "Land lease and port authority permits",
        },
        {
          id: "G4_OFFTAKE_BANKABLE",
          ...
          verified_count: 3,
          is_complete: false,
          blocking_items: ["offtake_15_year_term", "credit_support_deed"],
          description:
            "Long-term take-or-pay with investment-grade counterparty",
        },
        {
          id: "G8_AUDIT_GRADE_MODEL",
          name: "Audit-Grade Financial Model",
          ...
          blocking_items: [
            ...
            "independent_engineer_sign_off",
          ],
          description: "Third-party verified model with stress tests",
        },
      ],
      capital_status: [
        {
        ...
        },
        {
          ...
          amount: "€45M",
          is_unlocked: false,
          gating_gates: ["G0_SITE_RIGHTS", "G4_OFFTAKE_BANKABLE"],
          progress_pct: 62,
          commitment_status: "INDICATIVE",
        },
        {
          ...
          amount: "€195M",
          is_unlocked: false,
          gating_gates: ["G8_AUDIT_GRADE_MODEL", "G4_OFFTAKE_BANKABLE"],
          progress_pct: 55,
          commitment_status: "NONE",
        },
      ],
  ...
  // ── 3 · e-Methanol · San Sebastián, Spain ────────────────────────────────
  {
    ...
    description:
      "e-Methanol synthesis combining green H2 and biogenic CO2 captured from a nearby pulp mill. Primary offtake: Maersk and CMA CGM marine fuel blending.",
      ...
      gates: [
        {
          ...
          description:
            "Long-term take-or-pay with investment-grade counterparties",
        },
        {
          id: "G7_INSURANCE_BOUND",
          ...
          description: "Comprehensive construction & operational insurance",
        },
        {
          id: "G8_AUDIT_GRADE_MODEL",
          name: "Audit-Grade Financial Model",
          ...
          description: "Third-party verified model with stress testing",
        },
        {
          id: "G10_FINANCIAL_CLOSE_CP",
          ...
          blocking_items: [
            ...
            "equity_funding_confirmed",
          ],
          description: "All funding commitments and legal docs ready",
        },
      ],
      capital_status: [
        {
  ...
  // ── 4 · SAF · Wales, UK ──────────────────────────────────────────────────
  {
    ...
    description:
      "Power-to-liquid SAF via Fischer-Tropsch synthesis, co-located with former steelworks site. Offtake discussions with British Airways and easyJet.",
    owner_company: "Celtic Green Fuels Ltd",
    associated_companies: ["Zürich Versicherung AG"],
    bankability: {
      overall_completion: 38,
      next_milestone:
        "Advance permitting (G0) and technology wrap (G5) to attract institutional equity",
      unlocked_capital: ["GRANTS_TA"],
      risk_alerts: [
      ...
      gates: [
        {
          ...
          is_complete: false,
          blocking_items: [
            "full_planning_permission",
            "environmental_impact_assessment",
          ],
          description: "Brownfield site permitting and title transfer",
        },
        {
          id: "G3_INPUTS_SECURED",
          name: "Technology Vendor Locked",
          ...
          is_complete: false,
          blocking_items: [
            "technology_licence_agreement",
            "licensor_performance_guarantee",
            "technology_wrap_insurance",
          ],
          description: "Fischer-Tropsch technology licence and wrap",
        },
        {
          id: "G4_OFFTAKE_BANKABLE",
          name: "Bankable Off-take Secured",
          ...
          is_complete: false,
          blocking_items: [
            "signed_offtake_contract",
            "credit_support_deed",
            "offtake_15_year_term",
            "price_floor_mechanism",
          ],
          description: "Airline take-or-pay agreement and credit support",
        },
      ],
      capital_status: [
        {
  ...
  // ── 5 · e-NG · Le Havre, France ──────────────────────────────────────────
  {
    ...
    description:
      "Synthetic methane (e-NG) via Sabatier methanation from green H2 and offshore-sourced biogenic CO2. Grid injection under the French green gas tariff scheme.",
    owner_company: "Normandie Hydrogène SA",
    associated_companies: ["Allianz", "Zürich Versicherung AG"],
    bankability: {
      overall_completion: 91,
      next_milestone:
        "Certify ISO-14064 verification report (G11) to close DSRA escrow",
      unlocked_capital: [
        "GRANTS_TA",
        "SEED_VC_ANGEL",
        "STRATEGIC_EQUITY",
      ...
      gates: [
        {
          ...
          is_complete: true,
          blocking_items: [],
          description: "Port concession and grid injection permits",
        },
        {
          id: "G4_OFFTAKE_BANKABLE",
          name: "Bankable Off-take Secured",
          ...
          is_complete: true,
          blocking_items: [],
          description: "20-year grid injection contract with Engie",
        },
        {
          id: "G8_AUDIT_GRADE_MODEL",
          name: "Audit-Grade Financial Model",
          ...
          is_complete: true,
          blocking_items: [],
          description: "Big-4 certified model, Q4 2025",
        },
        {
          id: "G11_COD_STABILIZATION",
          name: "GHG Certification Verified",
          completion_pct: 60,
          total_evidence: 5,
          verified_count: 3,
          is_complete: false,
          blocking_items: [
            "iso_14064_verification_report",
            "third_party_ghg_audit",
          ],
          description: "ISO-14064 GHG verification and registry issuance",
        },
      ],
      capital_status: [
        {
  ...
  // ── 6 · e-Methanol · Hamburg, Germany — HamburgOne ───────────────────────
  {
    ...
    description:
      "Green e-methanol plant co-located with Hamburg port industrial zone. Primary offtake target: BremenThree AG (3,000 MT/month). Certifier: DNV. Team: Lisa Friedrich (CEO/CFO), Mark Puntz (Chief Engineer), Lucie Mertz (CCO). Structured lending via NordLB (Henrik Vost).",
    ...
    bankability: {
      overall_completion: 52,
      next_milestone:
        "Complete FEED and secure DNV interim certification milestone",
      unlocked_capital: ["GRANTS_TA"],
      risk_alerts: [
      ...
      gates: [
        {
          ...
          is_complete: true,
          blocking_items: [],
          description: "Hamburg port zone land lease signed",
        },
        {
          id: "G1_GRID_UTILITIES_REALITY",
          name: "Power, Water & Critical Utility Access",
          ...
          description:
            "GRID_CONNECTED · No PPA recorded — DEAL KILLER: electricity price unhedged, RFNBO/45V additionality cannot be evidenced. Grid interconnection study submitted.",
        },
        {
          id: "G3_INPUTS_SECURED",
          name: "Technology Vendor Locked",
          ...
          verified_count: 3,
          is_complete: false,
          blocking_items: ["epc_lump_sum_contract"],
          description:
            "e-MeOH reactor OEM selected; EPC contract in negotiation",
        },
        {
          id: "G4_OFFTAKE_BANKABLE",
          name: "Bankable Offtake",
          completion_pct: 30,
          total_evidence: 5,
          verified_count: 1,
          is_complete: false,
          ...
          description:
            "BremenThree AG target: 3,000 MT/month — term sheet stage",
        },
        {
          id: "G7_INSURANCE_BOUND",
          name: "Insurance Bound",
          completion_pct: 20,
          total_evidence: 3,
          verified_count: 0,
          is_complete: false,
          blocking_items: ["car_ear_bound", "dsu_quote"],
          description: "CAR/EAR/DSU to be placed on FEED completion",
        },
      ],
      capital_status: [
        {
      ...
      gates: [
        {
          ...
          is_complete: false,
          blocking_items: ["environmental_impact_assessment"],
          description: "Basque Country industrial zone — EIA in progress",
        },
        {
          id: "G1_GRID_UTILITIES_REALITY",
          ...
          verified_count: 1,
          is_complete: false,
          blocking_items: ["red_electrica_capacity", "offgrid_ppa_signed"],
          description:
            "Off-grid PPA preferred — Red Eléctrica capacity as backup",
        },
        {
          id: "G3_INPUTS_SECURED",
          name: "Technology Vendor Locked",
          ...
          verified_count: 2,
          is_complete: false,
          blocking_items: ["epc_selection", "co2_source_contract"],
          description: "EngineSpain Spa shortlisted as EPC — MoU signed",
        },
        {
          id: "G4_OFFTAKE_BANKABLE",
          name: "Bankable Offtake",
          ...
          description:
            "RotterdamOfftake4 AG: 10,000 MT/month. Pricing reference needed.",
        },
      ],
      capital_status: [
        {
          type: "GRANTS_TA",
          name: "Grants & TA (CEF/IDAE)",
          amount: "€15M",
          is_unlocked: true,
          gating_gates: [],
          progress_pct: 100,
  ...
  // financing (lender gates waived by the engine), internal self-offtake.
  {
    ...
    bankability: {
      ...
      next_milestone:
        "Complete BTM yield study and water permit pathway for corporate FID",
      unlocked_capital: ["GRANTS_TA"],
      risk_alerts: [],
      gates: [
        {
          ...
          is_complete: true,
          blocking_items: [],
          description: "Owned industrial land — site control inherent",
        },
        {
          id: "G1_GRID_UTILITIES_REALITY",
          name: "Power, Water & Critical Utility Access",
          ...
          description:
            "HYBRID — both grid and BTM tracks apply. Internal sleeved PPA signed; yield study and water permits outstanding",
        },
        {
          id: "G3_INPUTS_SECURED",
          name: "Key Inputs Secured",
          completion_pct: 70,
          total_evidence: 3,
          verified_count: 2,
          is_complete: false,
          blocking_items: ["logistics_concept_note"],
          description: "On-site consumption — minimal logistics",
        },
      ],
      capital_status: [
        {
          type: "GRANTS_TA",
          name: "IPCEI / NRW Grants",
          amount: "€28M",
          is_unlocked: true,
          gating_gates: [],
          progress_pct: 100,
