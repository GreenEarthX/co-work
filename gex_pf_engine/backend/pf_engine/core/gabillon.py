...
def tenor_label(tenor_months: int) -> str:
    """Display label for a tenor: whole years as "NY", everything else in months.

    18M stays "18M" — integer-dividing it to "1Y" collides with the 12M point.
    Mirrored by tenorLabel() in the platform frontend (features/pricing).
    """
    if tenor_months >= 12 and tenor_months % 12 == 0:
        return f"{tenor_months // 12}Y"
    ...
    def term_structure(
        ...
        for tenor_m in tenors_months:
            ...
            curve.append({
                ...
                "tenor_label": tenor_label(tenor_m),
                "price_eur": fwd,
