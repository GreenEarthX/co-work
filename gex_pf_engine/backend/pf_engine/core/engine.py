...
class ProjectFinanceEngine:
    ...
    def calculate_project_metrics(
        ...
        metrics = {
            ...
            "covenant_compliance": {
                # An UNDEFINED dscr (no debt service) must not read as passing.
                "dscr_compliant": dscr_guard.meets(dscr, 1.3),  # Typical covenant
                "dscr_defined": dscr_guard.is_defined(dscr),
                "dscr_value": dscr_guard.for_output(dscr),
                "dscr_threshold": 1.3
            }
        }
    ...
    def check_covenants(
        ...
        if 'dscr_minimum' in covenant_requirements:
            ...
            # Undefined (inf/None) is neither compliant nor a breach — it is
            # unknown, and unknown must not pass a covenant test.
            compliant = dscr_guard.meets(dscr_actual, dscr_required)
            
            checks.append({
                "covenant": "DSCR",
                "required": dscr_required,
                "actual": dscr_guard.for_output(dscr_actual),
                "compliant": compliant,
                "severity": "critical" if not compliant else "ok",
                "reason": (None if dscr_guard.is_defined(dscr_actual)
                           else dscr_guard.UNDEFINED_REASON)
            })
    ...
    def model_project_lifetime(
        ...
        """
        # Simple annual model
        # capacity_mtpd is metric TONNES/day → ×1000 to get kg (price/opex are
        # per kg). This mirrors cfads.py:57-58, which is the canonical form; the
        # ×1000 was fixed there and this second copy was missed, so revenue and
        # opex were both understated by a factor of 1000. The variable name said
        # kg while the value was tonnes.
        # NOTE: no capacity factor is applied — 365 asserts the plant runs every
        # day of the year. cfads.py exposes this as `period_days`; this engine
        # does not, so its output is a nameplate ceiling, not an expectation.
        annual_production_kg = capacity_mtpd * 1000 * 365
        ...
        for year in range(operations_start_year, operations_start_year + tenor_years):
            ...
            years.append({
                ...
                "dscr": dscr,
                "covenant_compliant": dscr_guard.meets(dscr, 1.3)
            })
