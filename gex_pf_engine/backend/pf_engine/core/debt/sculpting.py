...
from pf_engine.core import dscr_guard
...
class DebtSculptor:
    ...
    def sculpt(self, cfads_profile: List[float]) -> Dict:
        ...
        """
        # ── Reject a horizon longer than the debt life ──────────────────────
        # Sculpting past the final maturity is not a harder question, it is a
        # meaningless one: with no debt service, DSCR = CFADS / 0 is undefined.
        # The engine used to answer `inf` for those years, which poisoned
        # avg_dscr and max_dscr to `inf` and emitted `Infinity` — not valid JSON.
        #
        # Silently trimming the horizon would be worse: the caller would get a
        # summary over a period they did not ask about and never be told. So the
        # input is refused, and the message says exactly what to pass instead.
        debt_life = max((t.tenor for t in self.financing.tranches), default=0)
        years = len(cfads_profile)
        if debt_life and years > debt_life:
            raise ValueError(
                f"CFADS horizon of {years} years exceeds the debt life of "
                f"{debt_life} years (longest tranche tenor). Debt service is "
                f"zero from year {debt_life + 1} onward, so DSCR is undefined "
                f"there — not infinite — and any average or maximum across the "
                f"full horizon would be meaningless.\n"
                f"Pass the first {debt_life} CFADS values to sculpt the debt, "
                f"and model post-maturity cashflow separately: after the loan "
                f"is repaid there is no coverage ratio to compute."
            )
        ...
        for i, cfads in enumerate(cfads_profile):
            ...
            sculpted_year = SculptedYear(
                ...
                is_lock_up=dscr_guard.breaches(dscr, self.constraints.lock_up_dscr),
                is_compliant=dscr_guard.meets(dscr, self.constraints.min_dscr),
