...
from pf_engine.routers.pricing import router as pf_pricing_router
from pf_engine.api.validation import finite_safe_validation_handler

try:
    from pf_engine.api import routes_model
...
try:
    from pf_engine.api import routes_bankability
...
try:
    from pf_engine.api import routes_pricing
...
from pf_engine.auth.gex_jwt import public_path_guard
...
# `Infinity`/`NaN` in a request body used to make the 422 response itself
# fail to serialise, so the caller got a bare 500 with no validation detail.
# Measured 2026-09-09 on POST /api/v1/model/covenants/check with dscr=1e400.
app.add_exception_handler(RequestValidationError, finite_safe_validation_handler)
